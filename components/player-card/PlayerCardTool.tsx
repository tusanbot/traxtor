...
export default function PlayerCardTool() {
  ...
  const [customLabel, setCustomLabel] = useState("گل");
  const [customValue, setCustomValue] = useState("");
  const [image, setImage] = useState("");
  const [logo, setLogo] = useState("");
  const [downloading, setDownloading] = useState(false);

  const reset = () => { setName("نام بازیکن"); setNumber("۷"); setPosition("هافبک"); setTeam("تراکتور"); setRating("۸.۸"); setNationality("ایران"); setCustomLabel("گل"); setCustomValue(""); setImage(""); setLogo(""); };
  const readFile = (file: File, setter: (value: string) => void) => { const reader = new FileReader(); reader.onload = () => setter(String(reader.result)); reader.readAsDataURL(file); };
  const download = async () => { if (!ref.current) return; setDownloading(true); try { const url = await toPng(ref.current, { width: 1080, height: 1350, pixelRatio: 2, cacheBust: true }); const a = document.createElement("a"); a.download = "tractor-player-card.png"; a.href = url; a.click(); } finally { setDownloading(false); } };
      ...
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[390px_1fr]">
        <section className="rounded-3xl border border-white/10 bg-[#111] p-5">
          ...
          <div className="mt-6 space-y-3">
            ...
            <div className="rounded-2xl border border-[#d4a72c]/20 bg-[#d4a72c]/5 p-3"><div className="mb-2 text-xs font-bold text-[#d4a72c]">فیلد آماری اختیاری</div><div className="grid grid-cols-2 gap-2"><input value={customLabel} onChange={(e) => setCustomLabel(e.target.value)} placeholder="عنوان مثل گل" className="rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none" /><input value={customValue} onChange={(e) => setCustomValue(e.target.value)} placeholder="مقدار مثل ۵" className="rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none" /></div></div>
        ...
        <section className="flex justify-center rounded-3xl border border-white/10 bg-[#0d0d0d] p-3 md:p-6"><div className="w-full max-w-[540px] overflow-auto rounded-2xl bg-black p-2"><div ref={ref} style={{ width: 1080, height: 1350, transform: "scale(.5)", transformOrigin: "top left", marginBottom: -675 }} className="relative overflow-hidden bg-[#080808] text-white">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_75%_15%,rgba(215,25,32,.48),transparent_28%),radial-gradient(circle_at_20%_90%,rgba(212,167,44,.18),transparent_30%),linear-gradient(150deg,#070707_30%,#25090c_100%)]" />
          <div className="absolute inset-0 opacity-[.08]" style={{ backgroundImage: "linear-gradient(115deg,transparent 0 38%,#d71920 38% 39%,transparent 39% 100%),linear-gradient(65deg,transparent 0 62%,#d4a72c 62% 63%,transparent 63% 100%)" }} />
          <div className="absolute -left-40 top-40 h-[850px] w-[850px] rounded-full border-[100px] border-[#d71920]/10" /><div className="absolute -right-48 -top-48 h-[750px] w-[750px] rounded-full border-2 border-[#d4a72c]/20" />
          <div className="absolute right-0 top-0 h-full w-[18px] bg-[#d71920]" />
          <div className="relative flex h-full flex-col px-[72px] py-[65px]">
            <div className="flex items-start justify-between"><div><div className="text-[24px] font-black tracking-[8px] text-[#d4a72c]">TRACTOR SC</div><div className="mt-4 text-[68px] font-black leading-none">PLAYER CARD</div></div>{logo ? <img src={logo} alt="" className="h-[120px] w-[120px] rounded-full border-4 border-[#d4a72c] bg-white/5 object-contain p-2" /> : <div className="flex h-[120px] w-[120px] items-center justify-center rounded-full border-4 border-[#d71920] text-[44px] font-black">T</div>}</div>
            <div className="absolute left-[55px] top-[220px] h-[720px] w-[560px] overflow-hidden"><div className="absolute inset-x-0 bottom-0 mx-auto h-[610px] w-[610px] max-w-full rounded-full border border-[#d71920]/20 bg-[#d71920]/5" />{image ? <img src={image} alt="" className="absolute bottom-0 left-1/2 z-10 h-[610px] w-[510px] -translate-x-1/2 rounded-full object-cover object-top ring-[10px] ring-[#d71920]/40" /> : <div className="absolute bottom-0 left-1/2 z-10 flex h-[420px] w-[420px] -translate-x-1/2 items-center justify-center rounded-full border-8 border-[#d71920]/30 text-[180px] font-black text-white/10">{faDigits(number)}</div>}</div>
            <div className="absolute left-[65px] top-[850px] text-[150px] font-black leading-none text-[#d4a72c]/15">{faDigits(number)}</div>
            <div className="absolute right-[72px] top-[360px] z-20 w-[450px] text-right"><div className="text-[28px] font-bold text-[#d4a72c]">{position || "پست بازیکن"}</div><div className="mt-3 text-[72px] font-black leading-tight">{name || "نام بازیکن"}</div><div className="mt-2 text-[28px] text-white/45">{team || "تراکتور"} • {nationality || "ایران"}</div>
